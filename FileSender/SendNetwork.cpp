...
int startConnection(char* ip, int port)
{
    ...
    iResult = bind(RecvSocket, (SOCKADDR *) & SendAddr, sizeof (RecvAddr));
    if (iResult != 0) {
...
void handleCloseHandshake()
{
	...
	// we only send and receive handshake 10 times
	do
	{
		sendHandshake(MODE_COMPLETE);
		i++;
	}
	while (recvHandshake() != MODE_COMPLETE && i < 10);
